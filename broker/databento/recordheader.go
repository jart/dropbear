...
type RecordHeader struct {
	Length       uint8       // 0: record length in 32-bit words (e.g., 14 = 56 bytes for MBO)
	RType        RType       // 1: record type (0xa0 = MBO, 0x01 = Mbp1, etc.)
	PublisherID  Publisher   // 2: identifies data source
	InstrumentID uint32      // 4: numeric instrument identifier
	TSEvent      clocky.Time // 8: exchange timestamp in nanoseconds since UNIX epoch
}
