...
// TradeCond is a bitmask of trade conditions from the SIP feed.
// Multiple conditions can be OR'd together.
type TradeCond uint64

// Individual trade condition bits. Each corresponds to a single-character code.
const (
	TradeCondRegularSaleCTA     TradeCond = 1 << iota // code ' ' - Regular Sale (CTA)
...
var tradeCondFromChar [256]TradeCond
...
func (c TradeCond) GoString() string {
	if c == 0 {
		return "0"
	}
	var parts []string
	for _, cond := range []struct {
		...
		{TradeCondRegularSaleCTA, "sip.TradeCondRegularSaleCTA"},
		{TradeCondRegularSale, "sip.TradeCondRegularSale"},
		{TradeCondStoppedStock, "sip.TradeCondStoppedStock"},
		{TradeCondDerivativelyPriced, "sip.TradeCondDerivativelyPriced"},
		{TradeCondReopening, "sip.TradeCondReopening"},
		{TradeCondClosing, "sip.TradeCondClosing"},
		{TradeCondQCT, "sip.TradeCondQCT"},
		{TradeCondReserved, "sip.TradeCondReserved"},
		{TradeCondCorrectedClose, "sip.TradeCondCorrectedClose"},
		{TradeCondAcquisition, "sip.TradeCondAcquisition"},
		{TradeCondB, "sip.TradeCondB"},
		{TradeCondCash, "sip.TradeCondCash"},
		{TradeCondDistribution, "sip.TradeCondDistribution"},
		{TradeCondE, "sip.TradeCondE"},
		{TradeCondISO, "sip.TradeCondISO"},
		{TradeCondBunchedSold, "sip.TradeCondBunchedSold"},
		{TradeCondPriceVariation, "sip.TradeCondPriceVariation"},
		{TradeCondOddLot, "sip.TradeCondOddLot"},
		{TradeCondRule127, "sip.TradeCondRule127"},
		{TradeCondSoldLast, "sip.TradeCondSoldLast"},
		{TradeCondOfficialClose, "sip.TradeCondOfficialClose"},
		{TradeCondNextDay, "sip.TradeCondNextDay"},
		{TradeCondOpening, "sip.TradeCondOpening"},
		{TradeCondPriorRefPrice, "sip.TradeCondPriorRefPrice"},
		{TradeCondOfficialOpen, "sip.TradeCondOfficialOpen"},
		{TradeCondSeller, "sip.TradeCondSeller"},
		{TradeCondSplit, "sip.TradeCondSplit"},
		{TradeCondExtendedHours, "sip.TradeCondExtendedHours"},
		{TradeCondExtendedHoursOOS, "sip.TradeCondExtendedHoursOOS"},
		{TradeCondContingent, "sip.TradeCondContingent"},
		{TradeCondAveragePriceUTP, "sip.TradeCondAveragePriceUTP"},
		{TradeCondCross, "sip.TradeCondCross"},
		{TradeCondYellowFlag, "sip.TradeCondYellowFlag"},
		{TradeCondSoldOOS, "sip.TradeCondSoldOOS"},
	} {
		if c&cond.mask != 0 {
	...
	if len(parts) == 0 {
		return fmt.Sprintf("sip.TradeCond(0x%x)", uint64(c))
	}
	return strings.Join(parts, "|")
}

func (c *TradeCond) UnmarshalJSON(data []byte) error {
	// Parse JSON array like ["@","F"]
	if len(data) < 2 {
		return ErrInvalidTradeCond
	}
...
func (c TradeCond) MarshalJSON() ([]byte, error) {
	if c == 0 {
