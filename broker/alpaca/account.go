...
type Account struct {
	...
	BODDTBP               decimal.Decimal     `json:"bod_dtbp"`                    // buying power at the beginning of the day
	...
	UserConfigurations    UserConfigurations  `json:"user_configurations"`         //
	AdminConfigurations   AdminConfigurations `json:"admin_configurations"`        //
}

type AdminConfigurations struct {
	AllowInstantACH     bool            `json:"allow_instant_ach"`     // whether or not instant ACH is authorized for the account
	MaxMarginMultiplier decimal.Decimal `json:"max_margin_multiplier"` // maximum buying power multiplier user is authorized to have on this account, e.g. 4
}
