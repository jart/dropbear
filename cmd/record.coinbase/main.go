...
func newRecorder(client *coinbase.Client, symbol string) *recorder {
	home := os.Getenv("HOME")
	outputDir := home + "/marketdata/" + *flagName + "/coinbase/"
	os.MkdirAll(outputDir, 0755)
	outputPath := outputDir + strings.ToUpper(symbol)
	log.Printf("recording coinbase market data for %s to %s", symbol, outputPath)
...
func (r *recorder) run() {
	for tick := range coinbase.MarketData(r.symbol, r.client) {
		r.lock.Lock()
		if r.done {
			r.lock.Unlock()
			return
		...
		r.lock.Unlock()
		if err != nil {
			loggy.Fatalf("coinbase[%s]: market data write error: %v", r.symbol, err)
		}
	}
}
